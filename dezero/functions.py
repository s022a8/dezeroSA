...
class SoftmaxCrossEntropy(Function):
    ...
    def backward(self, gy):
        ...
        gy *= 1/N
        y = softmax(x)
        t_onehot = np.eye(CLS_NUM, dtype=t.dtype)[t.data]  # 単位行列を作成後、t.dataが示す行を順番に取得していくことで、onehotデータを作成できる。
        y = (y - t_onehot) * gy
        return y
...
class Clip(Function):
    ...
    def backward(self, gy):
        x, = self.inputs
        mask = (x.data >= self.x_min) * (x.data <= self.x_max)  # x_min以上、x_max以下のデータのみをgyから抽出する。
        gx = gy * mask
        return gx
